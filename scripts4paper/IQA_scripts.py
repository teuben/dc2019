...
## Created: A. Hacar, IQA group, D. Petry
##
#########################################################
## Functions
...
## IQA colours
IQA_colours = ["red", "blue", "orange", "green" , "cyan", "pink", "brown"]
...
def mask_image(myimage,threshold=0.0,relative=False):
        ...
        # Create your mask
        myia.open('masked.tmp')
        if (relative == False):
                myia.calcmask(mask= 'masked.tmp >= '+str(threshold), name='mymask')
        if (relative == True):
                ima_sigma = imstat(myimage)["rms"][0]
                myia.calcmask(mask= 'masked.tmp >= '+str(threshold*ima_sigma), name='mymask')
        myia.done()
...
def Compare_Apar(ref_image = '',target_image=[''],save=False):
        ...
        for m in np.arange(Nplots):
                ...
                Adist = Adist[(Adist <= 10.) & (Adist >= -10.)] # Just remove big outlayers
                skewness = np.round(skew(Adist),3)
                kurt = np.round(kurtosis(Adist),3)
                # Plot results
                plt.plot(mids,h,label=target_image[m] + "; A = "+ str(meanvalue) + " +/- " + str(sigmavalue),linewidth=3)
...
def Compare_Fidelity(ref_image = '',target_image=[''],save=False):
        ...
        for m in np.arange(Nplots):
                ...
                plt.plot(mids,h,label=target_image[m] + "; <Fidelity> = "+ str(meanvalue),linewidth=3)
                # Display on screen
...
def show_Apar_map(ref_image,target_image,channel=0):
        ...
        # get min/max values
        ##vmin, vmax = np.min(image[0].data[-(np.isnan(image[0].data).astype(int))]), np.max(image[0].data[-(np.isnan(image[0].data).astype(int))])
        vmin, vmax = np.min(image[0].data), np.max(image[0].data)
        # Continuum or cube?
...
def show_Fidelity_map(ref_image,target_image,channel=0):
        ...
        image = fits.open(ref_image+".fits")
        # get min/max values
        ##vmin, vmax = np.min(image[0].data[-(np.isnan(image[0].data).astype(int))]), np.max(image[0].data[-int(np.isnan(image[0].data).astype(int))])
        vmin, vmax = np.min(image[0].data), np.max(image[0].data)
        # Continuum or cube?
        Ndims = np.shape(np.shape(image[0].data))
        if (Ndims[0] == 2):

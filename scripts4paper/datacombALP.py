...
def runWSM(vis, sdimage, imname, spw='', field='', specmode='mfs', 
    ...
    if usemask == 'auto-multithresh':
        mymask = usemask
    elif os.path.exists(usemask):
        mymask = 'user'
        mask = usemask
    else:
        print('mask '+usemask+' does not exist')
        return False
    ...
    CellSizeArcsec = np.abs(imhead(mysdimage)['incr'][0])*206265. #in Arcsec
    ...
    runtclean(Int_vis,imname, startmodel=scaled_name,
                phasecenter=phasecenter, 
                spw='0', field=field, imsize=imsize, cell=cellsize,threshold=threshold,
                niter=niter,usemask='auto-multithresh')
